...
//-----------Customise (FOR USER)
interact_key		= global.ACTION;
interact_gamepad	= global.GP_ACTION;
up_key				= global.CONTROL_UP;		
down_key			= global.CONTROL_DOWN;

xscale				= srpt_xscale() * 3;
yscale				= srpt_yscale() * 3;
x_buffer			= 8 * xscale;
y_buffer			= 15 * yscale;
...
boxHeight		= sprite_get_height(dialogue_box) * yscale;
boxWidth		= sprite_get_width(dialogue_box) * xscale;
gui_width		= display_get_gui_width();
gui_height		= display_get_gui_height();
gb_diff			= gui_width - boxWidth;
portraitWidth	= sprite_get_width(portrait_frame) * xscale;
...
name_box_x		= pos_x + (8 * xscale);
name_box_y		= pos_y - (23 * yscale);
name_box_text_x = name_box_x + ((sprite_get_width(name_box)*xscale)/2);
