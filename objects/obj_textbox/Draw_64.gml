//Draw textbox
draw_sprite_ext(dialogue_box, 0, pos_x,pos_y, xscale,yscale, 0, c_white, 1);

//Draw portrait
if(portrait[page] != -1){
	draw_sprite_ext(portrait[page], emotion[page], (pos_x-portraitWidth), pos_y, xscale,yscale, 0, c_white, 1);
...
if(type[page] == 1 or charCount >= str_len) {
	if(portrait_idle[page] != -1){
		...
		if(portrait_idle_x[page] != -1){ posx += portrait_idle_x[page] * xscale; }
		if(portrait_idle_y[page] != -1){ posy += portrait_idle_y[page] * yscale; }
		
		portrait_idle_c += portrait_idle_s[page];
		if(portrait_idle_c >= portrait_idle_n[page]){ portrait_idle_c = 0; }
		draw_sprite_ext(portrait_idle[page], portrait_idle_c, posx, posy, xscale,yscale, 0, c_white, 1);	
	}
}
#endregion

	draw_sprite_ext(portrait_frame, 0, pos_x-portraitWidth, pos_y, xscale,yscale, 0, c_white, 1);
}
...
if(cname != "None"){
	//Draw namebox
	draw_sprite_ext(name_box, 0, name_box_x,name_box_y, xscale,yscale, 0, c_white, 1);
			...
			default:
				...
				if(portrait_talk[page] != -1) {
					if(!pause) {
						...
						draw_sprite_ext(portrait_talk[page], portrait_talk_c, posx, posy, xscale,yscale, 0, c_white, 1);	
					}
				} 
				#endregion
